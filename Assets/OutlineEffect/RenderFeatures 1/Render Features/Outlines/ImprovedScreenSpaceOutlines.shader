Shader "Hidden/ImprovedScreenSpaceOutlines"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float2 uv = input.uv;
                float4 normalSample = SAMPLE_TEXTURE2D(_FilterTexture, sampler_FilterTexture, uv);
                ...
                for (i = 0; i < 8; i++)
                {
                    normalSamples[i] = SAMPLE_TEXTURE2D(_FilterTexture, sampler_FilterTexture, offsets[i]);

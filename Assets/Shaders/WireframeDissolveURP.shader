         ...
         float3 Unity_NormalBlend(float3 A, float3 B)
{
 return normalize(float3(A.rg + B.rg, A.b * B.b));
         ...
         half4 frag(g2f i) : SV_Target
         {
             ...
             float3 _Ambient_ColorSky = SHADERGRAPH_AMBIENT_SKY.rgb;
             float3 _Ambient_Equator = SHADERGRAPH_AMBIENT_EQUATOR.rgb;
             float3 _Ambient_Ground = SHADERGRAPH_AMBIENT_GROUND.rgb;
             float3 combinedAmbient = _Ambient_ColorSky+_Ambient_Equator+ _Ambient_Ground;
